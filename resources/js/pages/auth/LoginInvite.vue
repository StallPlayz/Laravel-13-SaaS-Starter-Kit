...
defineOptions({
    layout: {
        ...
        description: 'Log in to your existing account to accept the workspace invitation.',
    },
});
...
<template>
    ...
    <div class="mb-6 text-center text-sm">
        You are joining <span class="font-semibold text-foreground">{{ invitation.workspace.name }}</span> as a {{ invitation.role }}.
    </div>

    <div class="mb-6 flex space-x-1 rounded-xl bg-muted p-1">
        <button @click="loginMethod = 'password'" :class="[
        ...
        <button @click="loginMethod = 'otp'" :class="[
            'w-full rounded-lg py-2.5 text-sm leading-5 font-medium',
            loginMethod === 'otp'
                ? 'bg-background shadow'
                : 'text-muted-foreground hover:bg-white/[0.12] hover:text-foreground',
    ...
    <form v-if="loginMethod === 'password'" @submit.prevent="submitPassword" class="flex flex-col gap-6">
        <div class="grid gap-6">
            <div class="grid gap-2">
                <Label for="email">Email address (Locked to invitation)</Label>
                <Input id="email" type="email" name="email" v-model="passwordForm.email" required disabled
                    class="bg-muted" />
            ...
            <div class="grid gap-2">
                ...
                <PasswordInput id="password" name="password" v-model="passwordForm.password" required autofocus
            ...
            <Button type="submit" class="mt-4 w-full" :tabindex="4" :disabled="passwordForm.processing"
    ...
    <form v-else-if="loginMethod === 'otp' && !otpSent" @submit.prevent="requestOtp" class="flex flex-col gap-6">
        <div class="grid gap-2">
            <Label for="otp_email">Email address</Label>
            <Input id="otp_email" name="email" type="email" v-model="otpForm.email" disabled class="bg-muted" required
    ...
    <form v-else @submit.prevent="verifyOtp" class="flex flex-col gap-6">
        <div class="grid gap-2 text-center">
            ...
            <div class="mt-4 flex justify-center">
                <InputOTP id="otp" v-model="otpForm.code" :maxlength="6" :disabled="otpForm.processing" autofocus>
                    <InputOTPGroup>
            ...
            <div class="mt-4 text-center text-sm">
                ...
                <button v-else @click.prevent="requestOtp" type="button"
